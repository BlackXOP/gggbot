...
TOKEN = 'MTM2MDUyMjI2NDg0MzM4Njk0MA.GDN1c4.QUFtFCxQ6WaCgYuEYs92LKoetp3ZZ7it-zAXjU'
...
def setup_vps_welcome(container_id: str, tier: str):
    """Sets up a welcome message in the container's .bashrc file"""
    label = "PAID" if tier == "paid" else "FREE"
    message = f"""
figlet "LS VPS"
echo "Welcome to LS VPS [{label}]"
echo "Thanks for {'buying' if tier == 'paid' else 'supporting'}, now enjoy!"
""".strip().replace('\n', ' && ')

    try:
        subprocess.run(["docker", "exec", container_id, "apt", "update"], check=True)
        subprocess.run(["docker", "exec", container_id, "apt", "install", "-y", "figlet"], check=True)
        subprocess.run([
            "docker", "exec", container_id, "bash", "-c",
            f"echo '{message}' >> /root/.bashrc"
        ], check=True)
    except subprocess.CalledProcessError as e:
        print(f"[ERROR] Failed to set up welcome message: {e}")

@bot.tree.command(name="deploy", description="Creates a new VPS instance with custom settings. (Admin Only)")
@app_commands.describe(
    ram="Amount of RAM (e.g., 3g, 8g)",
    cores="Number of CPU cores (e.g., 1, 2)",
    target_user="User ID to assign the VPS",
...
async def deploy_ubuntu(interaction: discord.Interaction, ram: str, cores: int, target_user: str, container_name: str):
    ...
    if admin_id not in {"1119657947434332211", "1085944828883369984"}:
    ...
    try:
        ...
        await interaction.response.send_message(
            embed=discord.Embed(description="❌ User not found.", color=0xff0000)
        )
        return
    ...
    try:
        ...
        setup_vps_welcome(container_id, tier="paid")
        ...
        if ssh_session_line:
            add_to_database(str(target_user), container_name, ssh_session_line)
            ...
            await interaction.followup.send(
                embed=discord.Embed(description="✅ VPS created and details sent to user.", color=0x00ff00)
            )
        else:
        ...
        await interaction.followup.send(
            embed=discord.Embed(description=f"❌ Error creating VPS: {e}", color=0xff0000)
        )
...
@bot.tree.command(name="create-vps", description="Create a 3GB RAM, 1-core VPS (for regular members)")
async def create_vps(interaction: discord.Interaction):
    userid = str(interaction.user.id)
    container_name = f"vps_{userid}_{int(time.time())}"

    # Enforce minimum VPS limit (if SERVER_LIMIT is defined globally)
    if count_user_servers(userid) >= SERVER_LIMIT:
        await interaction.response.send_message(
            embed=discord.Embed(
                description="🚫 **You have reached your VPS limit.**",
                color=0xff0000
            ),
            ephemeral=True
        )
    ...
    try:
        ...
        container_id = subprocess.check_output([
            "docker", "run", "-itd", "--privileged", "--hostname", "lsvps", "--cap-add=ALL",
            "-m", "3g", "--cpus", "1", "--name", container_name, "ubuntu-22.04-with-tmate"
        ]).strip().decode('utf-8')
        ...
        if ssh_session_line:
            ...
            await interaction.followup.send(
                embed=discord.Embed(
                    description="✅ **VPS created successfully. Check your DMs.**",
                    color=0x00ff00
                ),
                ephemeral=True
            )
